    ...
    impl HwVideoDecoder {
        pub fn new(config: DecoderConfig) -> Result<Self> {
            unsafe {
                ...
                let frame_memory =
                    CdramBlock::allocate("opennow_hw_video_frame", decoder_info.frameMemSize)?;
        ...
        /// Decodes one Access Unit into `direct_target` using the pixel format the render thread
        /// registered.
        pub fn decode(
            &mut self,
            access_unit: &[u8],
...
/// Host-build stand-in so `cargo check` for non-Vita targets still type-checks; the real binary
/// only ever ships for the Vita.
#[cfg(not(target_os = "vita"))]
pub struct HwVideoDecoder;
